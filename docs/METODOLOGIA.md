...
## Agregados v11
| Tema | Fuente y método | Límites |
|---|---|---|
| Proyección oficial 24-Month Study (Powell y Mead) | Gráficos PNG que publica USBR (`riverops/webreports/Powell24MS.png`, `Mead24MS.png`) + PDFs de los estudios | USBR no publica los resultados en formato de datos abierto (sólo PDF/imagen; el servicio HDB exige un ID de corrida interno que cambia cada mes). El monitor no los procesa ni los mezcla. |
| Pronósticos oficiales de otros embalses y ríos | NRCS AWDB `/forecasts` (SRVO) para todos los puntos de pronóstico de HUC 14/15 (Flaming Gorge, Blue Mesa, Navajo, McPhee, Granby, Dillon, Fontenelle…). Última publicación del año hidrológico, con su período. | Después de abril los períodos cambian (mayo–julio, junio–julio): sólo se comparan filas del mismo período. |
| Compact de 1922 (Lee Ferry, 10 años) | USGS 09380000 + 09382000, caudal medio diario × 1,983471 por año hidrológico; suma móvil de 10 años; referencias 75 y 82,5 MAF | Cálculo del monitor; años con < 99 % de días no se usan. La contabilidad oficial (USBR / UCRC) puede diferir. |
| Temperatura SNOTEL | TAVG / TMAX / TMIN (°F → °C) de los últimos 7 días; promedio histórico del día que publica NRCS; % de días con máxima > 0 °C | Indicador de condiciones de deshielo, no un modelo de fusión. |
| Nieve en toda la cuenca (SNODAS) | **No implementado.** | NOAA no ofrece un total por cuenca del Colorado vía API: sus regiones (Central Rockies, Southwest…) no coinciden con la cuenca; la grilla SNODAS diaria completa es demasiado pesada para una función de Netlify, y el servicio de mapas de NOHRSC devuelve valores de píxel sin unidades documentadas. Con la cuenca sin nieve (septiembre) no se pueden validar contra SNOTEL. Se puede retomar en invierno. |
